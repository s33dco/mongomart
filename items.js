...
var MongoClient = require('mongodb').MongoClient,
  ...
  // https://stackoverflow.com/questions/49972129/assert-in-mongodb-3-6-nodejs-driver-and-how-to-use-assert-when-using-promise-im


function ItemDAO(database) {
	// "use strict";
	this.db = database; // user this.db.collection('item')... for querys
	...
	this.getCategories = (callback) => {
      ...
      let totalProducts = categories.reduce((total, {num}) => total + num, 0);
      categories.unshift({ "_id": "All", "num": totalProducts});
			callback(categories);
		});
	};
	...
	this.getItems = (category, page, itemsPerPage, callback) => {
		...
		this.db.collection("item").aggregate(query).toArray((err, pageItems) => {
			assert.equal(err, null);
			callback(pageItems);
		});
	...
	this.searchItems = (query, page, itemsPerPage, callback) => {
		...
		this.db.collection("item").aggregate(searchQuery).toArray((err, items) => {
			assert.equal(err, null);
			callback(items);
		});
	};
	...
	this.getNumSearchItems = (query, callback) => {
		...
		this.db.collection("item").find({ $text: { $search: query } }).count((err, numItems) => {
			assert.equal(err, null);
			callback(numItems);
		});
	};
	...
	this.addReview = (itemId, comment, name, stars, callback) => {
     ...
     let product = { "_id": itemId };
     let update = { $push: { "reviews": review } };

     this.db.collection('item').updateOne(product, update, (err, item) => {
         assert.equal(err, null);
         callback(item);
     })
 }
