...
var MongoClient = require('mongodb').MongoClient,
    ...
    function CartDAO(database) {
        ...
        this.cartCollection = this.db.collection('cart'); //different format to item.js including collection name
        ...
        this.getCart = (userId, callback) => {
            ...
            */
            ...
            this.cartCollection.findOne(user, (err, cart) => {
        ...
        this.itemInCart = (userId, itemId, callback) => {
            ...
            this.cartCollection.findOne(queryDoc, { "items.$": 1 }, function(err, result) { // 'items.$' limits array to one
                assert.equal(err, null);
                if (result !== null) {
        ...
        this.addItem = (userId, item, callback) => {
            ...
            this.db.collection("cart").findOneAndUpdate(
                ...
                {"$push": {items: item}},
                {
                    // make cart if can't find one
                    upsert: true,
                    // return updated cart
                    returnOriginal: false
                },
                // updated document is the value attribute of result.
                (err, result) => {
        ...
        this.updateQuantity = (userId, itemId, quantity, callback) => {
            ...
            */
            ...
            let query = {
                userId: userId,           // userID in cart doc
                "items._id": itemId       // _id of product in items array in doc
            };
            let options = {               // set options for findOneAndUpdate so updated doc returned
                upsert: false,
                returnOriginal: false
            };

            // set up documents for findOneAndUpdate..

            let updateChangeQuantityItem = {               // if not removing from cart
                    $set: { "items.$.quantity": quantity }
            };
            let updateRemoveItem = { $pull: { "items": { _id: itemId } } };  // otherwise
            let update = {};

            if (quantity === 0) {                       // do we remove or change quantity
                update = updateRemoveItem;
            } else {
                update = updateChangeQuantityItem;
            }

            // findOneAndUpdate call

            this.db.collection('cart').findOneAndUpdate(query, update, options, (err, result) => {
                assert.equal(err, null);
                callback(result.value);
            });
        }
